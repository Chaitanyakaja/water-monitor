...
def get_data(interval):
    ...
    conn=sqlite3.connect('/ph/pH.db')
    curs=conn.cursor()

    if interval == None:
        curs.execute("SELECT * FROM phread")
    else:
        curs.execute("SELECT * FROM phread WHERE date>datetime(CURRENT_TIMESTAMP,'localtime','-%s hours')" % interval)
...
def show_stats(option):
    ...
    curs.execute("SELECT date,max(ph) FROM phread WHERE date>datetime(CURRENT_TIMESTAMP,'localtime','-%s hour') AND date<=datetime(CURRENT_TIMESTAMP,'localtime')" % option)
#    curs.execute("SELECT timestamp,max(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
    ...
    curs.execute("SELECT date,min(ph) FROM phread WHERE date>datetime(CURRENT_TIMESTAMP,'localtime','-%s hour') AND date<=datetime(CURRENT_TIMESTAMP,'localtime')" % option)
#    curs.execute("SELECT timestamp,min(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
    ...
    curs.execute("SELECT avg(ph) FROM phread WHERE date>datetime(CURRENT_TIMESTAMP,'localtime','-%s hour') AND date<=datetime(CURRENT_TIMESTAMP,'localtime')" % option)
#    curs.execute("SELECT avg(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
    ...
    print "<h2>Minumum pH&nbsp</h2>"
    print rowstrmin
    print "<h2>Maximum pH&nbsp</h2>"
    print rowstrmax
    print "<h2>Average pH</h2>"
    print "%.3f" % rowavg
    ...
    print "<table>"
    print "<tr><td><strong>Date/Time</strong></td><td><strong>pH</strong></td></tr>"

    rows=curs.execute("SELECT * FROM phread WHERE date>datetime(CURRENT_TIMESTAMP,'localtime','-1 hour') AND date<=datetime(CURRENT_TIMESTAMP,'localtime')")
...
def main():
    ...
    printHTMLHead("Aquaponics pH Logger", table)

    # print the page body
    print "<body>"
    print "<h1>Aquaponics pH Logger</h1>"
