...
# global variables
dbname='/temperature/temperature.db'
...
def get_data(interval):
    ...
    conn=sqlite3.connect('/temperature/temperature.db')
    curs=conn.cursor()

    if interval == None:
        curs.execute("SELECT * FROM temp")
    else:
        curs.execute("SELECT * FROM temp WHERE date>datetime(CURRENT_TIMESTAMP,'localtime','-%s hours')" % interval)
...
def show_stats(option):
    ...
    curs.execute("SELECT date,max(temp) FROM temp WHERE date>datetime(CURRENT_TIMESTAMP,'localtime','-%s hour') AND date<=datetime(CURRENT_TIMESTAMP,'localtime')" % option)
#    curs.execute("SELECT timestamp,max(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
    ...
    curs.execute("SELECT date,min(temp) FROM temp WHERE date>datetime(CURRENT_TIMESTAMP,'localtime','-%s hour') AND date<=datetime(CURRENT_TIMESTAMP,'localtime')" % option)
#    curs.execute("SELECT timestamp,min(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
    ...
    curs.execute("SELECT avg(temp) FROM temp WHERE date>datetime(CURRENT_TIMESTAMP,'localtime','-%s hour') AND date<=datetime(CURRENT_TIMESTAMP,'localtime')" % option)
#    curs.execute("SELECT avg(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
    ...
    print rowstrmin
    print "<h2>Maximum temperature&nbsp</h2>"
    print rowstrmax
    ...
    rows=curs.execute("SELECT * FROM temp WHERE date>datetime(CURRENT_TIMESTAMP,'localtime','-1 hour') AND date<=datetime(CURRENT_TIMESTAMP,'localtime')")
...
def print_time_selector(option):
    ...
    print """<form action="/cgi-bin/temperature.py" method="POST">
...
def main():
    ...
    printHTMLHead("Temperature Logs", table)

    # print the page body
    print "<body>"
    print "<h1>Temperature Logs</h1>"
